...
router.get('/products',  requireToken, (req, res, next) => {
	Product.find()
		.then((product) => {
			// `product` will be an array of Mongoose documents
			// we want to convert each one to a POJO, so we use `.map` to
			// apply `.toObject` to each one
			return product.map((product) => product.toObject())
		})
		// respond with status 200 and JSON of the product
...
// SHOW
// GET /product/5a7db6c74d55bc51bdf39793
...
// UPDATE
// PATCH /product/5a7db6c74d55bc51bdf39793
router.patch('/product/:id', requireToken, removeBlanks, (req, res, next) => {
...
// DESTROY
// DELETE /product/5a7db6c74d55bc51bdf39793
